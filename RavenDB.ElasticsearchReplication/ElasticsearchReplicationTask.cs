...
namespace Raven.Bundles.ElasticsearchReplication
{
    ...
    public class ElasticsearchReplicationTask : IStartupTask, IDisposable
    {
        ...
        public void Execute(DocumentDatabase database)
        {
            ...
            Database.OnDocumentChange += (sender, notification, metadata) =>
            {
                ...
                if (!notification.Id.StartsWith(ElasticsearchReplicationConstants.ElasticsearchReplicationConfig, StringComparison.InvariantCultureIgnoreCase))
                    return;

                replicationConfigs = null;
                statistics.Clear();
                log.Debug(() => "Elasticsearch Replication configuration was changed.");
            };
            ...
            var task = Task.Factory.StartNew(() =>
            {
                using (LogContext.WithDatabase(database.Name))
                {
                    try
                    {
                        BackgroundElasticsearchReplication();
                    }
                    catch (Exception e)
                    {
                        log.ErrorException("Fatal failure when replicating to Elasticsearch. All Elasticsearch Replication activity STOPPED", e);
                    }
                }
            }, TaskCreationOptions.LongRunning);
            database.ExtensionsState.GetOrAdd(typeof(ElasticsearchReplicationTask).FullName, k => new DisposableAction(task.Wait));
        }
        ...
        {
            var jsonDocument = Database.Get(ElasticsearchReplicationConstants.ElasticsearchReplicationStatus, null);
            return jsonDocument == null
                                    ? new SqlReplicationStatus()
                                    : jsonDocument.DataAsJson.JsonDeserialization<SqlReplicationStatus>();
        }

        private void BackgroundElasticsearchReplication()
        {
            int workCounter = 0;
            while (Database.WorkContext.DoWork)
            {
                ...
                if (config.Count == 0)
                {
                    Database.WorkContext.WaitForWork(TimeSpan.FromMinutes(10), ref workCounter, "Elasticsearch Replication");
                    continue;
                }
                ...
                try
                {
                    BackgroundTaskExecuter.Instance.ExecuteAllInterleaved(Database.WorkContext, relevantConfigs, replicationConfig =>
                    {
                        ...
                        {
                            log.WarnException("Error while replication to Elasticsearch destination: " + replicationConfig.Name, e);
                            Database.AddAlert(new Alert
                            {
                                AlertLevel = AlertLevel.Error,
                                CreatedAt = SystemTime.UtcNow,
                                Exception = e.ToString(),
                                Title = "Elasticsearch Replication failure to replication",
                                Message = "Elasticsearch Replication could not replicate to " + replicationConfig.Name,
                                UniqueKey = "Elasticsearch Replication could not replicate to " + replicationConfig.Name
                            });
                        }
                    });
        ...
        private List<SqlReplicationConfig> GetConfiguredReplicationDestinations()
        {
            ...
            Database.TransactionalStorage.Batch(accessor =>
            {
                const string prefix = ElasticsearchReplicationConstants.ElasticsearchReplicationConfig;
                                ...
                                prefix, 0, 256))
                {
                    ...
                    {
                        log.Warn("Could not find name for Elasticsearch replication document {0}, ignoring", document.Key);
                        continue;
                    }
...
//                        {
//                            log.Warn("Could not find connection string named '{0}' for Elasticsearch replication config: {1}, ignoring Elasticsearch replication setting.",
...
//                    }
                    else if (string.IsNullOrWhiteSpace(cfg.ConnectionStringSettingName) == false)
                    {
                        ...
                        {
                            log.Warn("Could not find setting named '{0}' for Elasticsearch replication config: {1}, ignoring Elasticsearch replication setting.",
                                cfg.ConnectionStringName,
                                document.Key);
                            continue;
                        }
